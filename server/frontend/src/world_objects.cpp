 ...
 * <File description>
 *
 * $Id: world_objects.cpp,v 1.1 2002/03/20 18:10:21 robofly Exp $
 */
...
void	CDynamicObject::load(std::string fileName)
{
	...
	try
	{
		...
		{
			nlinfo("reading dynamic object for %s\n", fileName.c_str());
			serial(f);
			f.close();
		}
		else
...
{
	nlinfo("Entering loadFromTextFile file.");
...
{
	nlinfo("Entering createFromTextFile file.");
